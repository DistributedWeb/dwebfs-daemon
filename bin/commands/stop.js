...
class StopCommand extends Command {
  static usage = 'stop'
  static description = 'Stop the DWebFs daemon.'
  static flags = {
    name: flags.string({
  ...
  async run () {
    ...
    const spinner = ora('Stopping the DWebFs daemon (might take a while to unannounce)...').start()
    try {
      await stop(flags.name, flags.port)
      spinner.succeed('The DWebFs daemon has been stopped.')
    } catch (err) {
      spinner.fail('Could not stop the DWebFs daemon:')
      console.error(err)
      this.exit(1)
    }
    this.exit()
