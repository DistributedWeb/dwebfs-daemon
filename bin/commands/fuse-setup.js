...
class SetupCommand extends Command {
  ...
  async run () {
    ...
    try {
      ...
      console.log('  * Your root drive will be mounted at ~/DWebFs when the daemon is next started.')
      console.log('  * If your mountpoint ever becomes unresponsive, try running `dwebfs force-unmount`.')
    } catch (err) {
      ...
      try {
        ...
        if (!mountpointStat) {
          ...
          try {
            ...
            // If DWebFs is a directory, this will error, but it doesn't matter.
          }
        }
      } catch (err) {
