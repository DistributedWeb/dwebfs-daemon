...
const HypercoreCache = require('ddatabase-cache')
...
const { rpc, apiVersion } = require('dwebfs-daemon-client')
...
class HyperdriveDaemon extends EventEmitter {
  constructor (opts = {}) {
    ...
    this._storageProvider = this.memoryOnly ? require('random-access-memory') : require('ddatabase-default-storage')
    ...
    this.dwebx = new Corestore(corestoreOpts.storage, corestoreOpts)
  ...
  async _ready () {
    ...
    await this.dwebx.ready()

    // Note: This API is not exposed anymore -- this is a temporary fix.
    const seed = this.dwebx.inner._deriveSecret(NAMESPACE, 'replication-keypair')
    const swarmId = this.dwebx.inner._deriveSecret(NAMESPACE, 'swarm-id')

    this._networkOpts.keyPair = HypercoreProtocol.keyPair(seed)
    this._networkOpts.id = swarmId

    this.networking = new SwarmNetworker(this.dwebx, this._networkOpts)
    ...
    // Register the Hyperswarm timeout heuristics on all cores generated by our dwebx.
    ...
    this.drives = new DriveManager(this.dwebx, this.networking, dbs.drives, {
      ...this.opts,
    ...
    this._versions = {
      ...
      client: require('dwebfs-daemon-client/package.json').version,
      schema: require('dwebfs-schemas/package.json').version,
      dwebfs: require('dwebfs/package.json').version
    }
  ...
  async _rpcStatus (call) {
    ...
    if (this._versions) {
      ...
      rsp.setHyperdriveversion(this._versions.dwebfs)
  ...
  async stop (err) {
    ...
    try {
      ...
      log.info('waiting for dwebx to close')
      if (this.dwebx) {
