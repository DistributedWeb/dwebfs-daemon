...
const { fromHyperdriveOptions, toHyperdriveOptions, toDriveInfo } = require('dwebfs-daemon-client/lib/common')
...
class FuseManager extends EventEmitter {
  ...
  _wrapHandlers (handlers) {
    ...
    // The RootListHandler/NetworkInfoHandler operate on the top-level DWebFs.
  ...
  async mount (mnt, mountOpts = {}) {
    ...
    async function mountSubdrive (relativePath, drive) {
      log.debug({ discoveryKey: drive.discoveryKey.toString('hex') }, 'mounting a sub-dwebfs')
  ...
  async mountDrive (path, opts) {
    if (!this._rootDrive) throw new Error('The root dwebfs must first be created before mounting additional drives.')
    if (!this._rootMnt || !path.startsWith(this._rootMnt)) throw new Error('Drives can only be mounted within the mountpoint.')

    // The dwebx name is not very important here, since the initial drive will be discarded after mount.
...
function ensureFuse () {
  return new Promise((resolve, reject) => {
    hyperfuse.isConfigured((err, configured) => {
      if (err) return reject(err)
      if (!configured) return reject(new Error('FUSE is not configured. Please run `dwebfs fuse-setup` first.'))
      return resolve()
    })
  })
}
