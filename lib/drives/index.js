...
const {
  ...
  setFileStats,
  toChunks
} = require('dwebfs-daemon-client/lib/common')
const { rpc } = require('dwebfs-daemon-client')
...
const TRIE_UPDATER_SYMBOL = Symbol('dwebfs-daemon-trie-updater')

class DriveManager extends Nanoresource {
  constructor (dwebx, networking, db, opts = {}) {
    super()
  ...
  async getNetworkConfiguration (discoveryKey) {
    ...
    const networkOpts = this._transientSeedIndex.get(encodedKey) || await dbGet(this._seedIndex, encodedKey)
  ...
  async get (key, opts = {}) {
    ...
    if (!drive) {
      ...
      const store = this.dwebx.namespace(randomNamespace)
      drive = dwebfs(store, key, {
        ...driveOpts
      })
    ...
    if (opts.fuseNetwork) {
      // TODO: The Network drive does not announce any settings for now.
      initialConfig = { lookup: true, announce: false }
  ...
  async _rpcPeerCounts (call) {
    ...
    for (let key of keys) {
      key = Buffer.from(key)
      if (this.dwebx.isLoaded(key)) {
        const core = this.dwebx.get(key)
  ...
  async _rpcStats (call) {
    ...
    const networkConfig = await this.getNetworkConfiguration(drive.discoveryKey)
  ...
  async _rpcMirror (call) {
    const id = call.request.getId()

    if (!id) throw new Error('A mirror request must specify a session ID.')
    const drive = this.driveForSession(id)
    await this._startMirroring(drive)

    const rsp = new rpc.drive.messages.MirrorResponse()
    return rsp
  }

  async _rpcUnmirror (call) {
    const id = call.request.getId()

    if (!id) throw new Error('An unmirror request must specify a session ID.')
    const drive = this.driveForSession(id)
    await this._stopMirroring(drive)

    const rsp = new rpc.drive.messages.UnmirrorResponse()
    return rsp
  }
